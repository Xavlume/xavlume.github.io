...
const INPUT_SCHEMA = [
  ...
  ["tab-re", 8, "fhsaAnnualLimit", "money"],
  ["tab-re", 9, "fhsaMaxBalance", "money"],
  ["tab-re", 10, "hbpMaxWithdrawal", "money"],
  ["tab-re", 11, "hbpRepaymentYears", "years"],
...
function applyModelToInputs(config) {
  ...
  INPUT_SCHEMA.forEach(([tabId, index, field, kind]) => {
    const element = inputs[tabId + ":" + index];
    if (!element) return;
...
function readModelInputs() {
  ...
  for (let index = 0; index < INPUT_SCHEMA.length; index++) {
    const [tabId, , field, kind] = INPUT_SCHEMA[index];
    const element = inputs[tabId + ":" + index];
    if (!element) continue;
...
async function createDeviceContext() {
  ...
  const promise = (async () => {
    if (!navigator.gpu) throw new Error("WebGPU is unavailable. Use a current Chrome or Edge browser with WebGPU enabled.");
    ...
    // binding.
    const quantilesShader = device.createShaderModule({code: QUANTILES_SHADER_SOURCE});
